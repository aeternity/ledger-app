...
// Conversion using `as` is done here because the string is constant and the
...
pub fn handler_sign_message(comm: &mut Comm) -> Result<(), AppSW> {
    ...
    let (message_length_bytes, message) = data
        .split_first_chunk::<4>()
        .ok_or(AppSW::MsgWrongLength)?;
    ...
    // TODO: implement view for message signing
    if true {
